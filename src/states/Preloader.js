...
PowerGym.States.Preloader.prototype = {
  ...
  preload: function () {
    ...
    this.preloadBar = this.add.image(0, 0, "preloaderBarBg")
    this.preloadBarFill = this.preloadBar.addChild(this.game.make.sprite(2.5, 2.5, "preloaderBar"));
    this.placePreloadBar();
  ...
  placePreloadBar: function() {

    this.preloadBar.scale.set(this.gameScale);
    this.preloadBar.x = this.game.width / 2 - this.preloadBar.width / 2;
    this.preloadBar.y = this.game.height / 2 - this.preloadBar.height / 2;
  ...
  update: function () {
    ...
    //  game experience. Basically it will wait for our audio file to be
    //  decoded before proceeding to the MainMenu. You can jump right into the
    //  menu if you want and still play the music, but you"ll have a few
    //  seconds of delay while the mp3 decodes - so if you need your music to
    //  be in-sync with your menu it"s best to wait for it to decode here
    //  first, then carry on.

    //  If you don"t have any music in your game then put the game.state.start
    //  line into the create function and delete the update function completely.
